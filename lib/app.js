var Promise, chalk, createDirectories, exif, exifdate, fs, getFileStat, getFiles, getPhotoDate, isJpg, isPhoto, log, mkdirp, moment, path, processError, readChunk, sortPhotos, _;
...
sortPhotos = function(directories, workingDirectory, outputDirectory, sortable) {
  return Promise.each(directories, function(dir) {
    return Promise.each(sortable[dir], function(file) {
      return fs.renameAsync("" + workingDirectory + "/" + file, "" + outputDirectory + "/" + dir + "/" + file);
    });
  });
};
...
module.exports = function(args, opts) {
  ...
  return getFiles(workingDirectory).filter(function(file) {
    return getFileStat(workingDirectory + '/' + file).then(function(fileStat) {
      if (fileStat.isFile()) {
        if (onlyPhotos) {
    ...
    return createDirectories(_.keys(sortable), outputDirectory);
