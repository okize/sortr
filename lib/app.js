var Promise, chalk, convertExifDate, exif, floorFigure, fs, getFileStat, getFiles, getPhotoDate, getProgressTime, isJpg, isPhoto, log, moment, path, processError, readChunk, sortPhotos, _;
...
sortPhotos = function(sortable, inputDirectory, outputDirectory, dryRun) {
  var directories;
  ...
  return Promise.each(directories, function(dir) {
    return Promise.each(sortable[dir], function(file) {
      ...
      if (!dryRun) {
        ...
        return log('dryRun', "will move " + file + " to " + outputDirectory + "/" + dir + "/");
      }
    });
  });
};
...
getProgressTime = function(timer) {
  var diff, time;
  diff = process.hrtime(timer);
  time = ((diff[0] * 1e9) + diff[1]) / 1e9;
  return floorFigure(time, 3);
};
...
module.exports = function(args, opts) {
  ...
  if (opts.verbose) {
    log('status', "- reading input directory files (" + (getProgressTime(timer)) + "s)");
  }
  return getFiles(inputDirectory).filter(function(file, i) {
    if (i === 0) {
      if (opts.verbose) {
        log('status', "- filtering non-photo files from sort set (" + (getProgressTime(timer)) + "s)");
      }
    }
    ...
    if (i === 0) {
      if (opts.verbose) {
        log('status', "- reading photo exif dates (" + (getProgressTime(timer)) + "s)");
      }
    }
    ...
    if (opts.verbose) {
      log('status', "- sorting photos into directories (" + (getProgressTime(timer)) + "s)");
    }
    if (_.isEmpty(sortable)) {
      return log('warning', 'No sortable photos found!');
    } else {
      if (!opts.dryrun) {
        return sortPhotos(sortable, inputDirectory, outputDirectory, false);
      } else {
        return sortPhotos(sortable, inputDirectory, outputDirectory, true);
      }
    }
  }).then(function() {
    var dirCount, photoCount;
    if (opts.verbose) {
      log('status', "- finished! (" + (getProgressTime(timer)) + "s)");
    }
